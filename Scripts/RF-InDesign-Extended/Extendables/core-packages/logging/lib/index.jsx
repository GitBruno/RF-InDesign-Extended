...
var Log = function (name, log_level) {
	...
	/**
	 * @desc cleans the contents of the log file
	 */
	this.clean = function () {
		var log = self.file;
		log.open("e");
	...
	this.wipe = this.clean;
	
	/**
	 * @desc log the details of the passed object
	 * @param {Object} object The object to log the details of
	 * @param {String} name an optional name of the variable passed
	 */
	this.details = function (object, name) {
		var log = self.file;
		var klass = object.constructor.name;
		if (klass == 'Object') {object = object.serialize('json')};
		if (name == undefined) {name = ""};
		var logmessage = "Var  : {varName}\nClass: {class}\nVal  : {varVal}\n".format({"varName":name, "class":klass, "varVal":object.toString()});//.replace(/^(.)/g, "\1      ")
		log.open("e");
		log.seek(log.length);	
		log.writeln(logmessage);
		log.close();		
	...
	/**
	 * @desc log a simple string
	 */
	this.dev = function (string) {
		var log = self.file;
		log.open("e");
		log.seek(log.length);	
		log.writeln(string);
